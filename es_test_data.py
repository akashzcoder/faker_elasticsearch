...
class CreateFakeData(object):
    ...
    def __init__(self, path_to_config: str) -> None:
        ...
        tornado.options.define("format", type=str, default='name:str,address:words,country_code:country_code,'
                                                           'account_number:account_number,ent_type:ent_type',
                               help="message format")
        ...
        tornado.options.define("force_init_index", type=bool, default=False,
                               help="Force deleting and re-initializing the "
                                    "Elasticsearch index")
        tornado.options.define("set_refresh", type=bool, default=False,
                               help="Set refresh rate to -1 before starting the "
                                    "upload")
        ...
        tornado.options.define("id_type", type=str, default=None,
                               help="Type of 'id' to use for the docs, valid settings "
                                    "are int and uuid4, None is default")
        ...
        tornado.options.define("validate_cert", type=bool, default=True,
                               help="SSL validate_cert for requests. Use false "
                                    "for self-signed certificates.")
        try:
            tornado.options.parse_config_file(path_to_config)
    ...
    def upload_batch(self, upload_data_txt):
        try:
    ...
    def get_data_for_format(self, format):
        ...
        if field_type == "bool":
        ...
        elif field_type == "ent_type":
            ent_types = ["company", "individual", "country", "region"]
            return_val = random.choice(ent_types)

        elif field_type == "account_number":
            lower = 10000000000000
            upper = 99999999999999
            return_val = str(self.generate_count(lower, upper))

        elif field_type == "country_code":
            codes = ["ma", "ca", "us", "ua", "ca", "in", "hk", "ph"]
            return_val = random.choice(codes)

        elif field_type == "str":
            min = 3 if len(split_f) < 3 else int(split_f[2])
            max = min + 7 if len(split_f) < 4 else int(split_f[3])
            length = self.generate_count(min, max)
            ...
            return_val = self.generate_count(min, max)

        elif field_type == "ipv4":
            return_val = "{0}.{1}.{2}.{3}".format(self.generate_count(0, 245), self.generate_count(0, 245), self.generate_count(0, 245), self.generate_count(0, 245))
            ...
            ts = self.generate_count(min, max)
            ...
            count = self.generate_count(min, max)
            words = []
    ...
    def generate_random_doc(self, format):
        global id_counter

        res = {}

        for f in format:
            f_key, f_val = self.get_data_for_format(f)
            if f_key:
                res[f_key] = f_val
    ...
    def generate_test_data(self):
        ...
        if tornado.options.options.force_init_index:
            self.delete_index(tornado.options.options.index_name)

        self.create_index(tornado.options.options.index_name)

        if tornado.options.options.set_refresh:
            self.set_index_refresh("-1")
        ...
        for num in range(0, tornado.options.options.count):
            ...
            item = self.generate_random_doc(format)
            ...
            if upload_data_count % tornado.options.options.batch_size == 0:
                yield self.upload_batch(upload_data_txt)
                upload_data_txt = ""
